...
our @EXPORT_OK = qw(get_media_objects add_media_object get_media_object get_media_object_file 
		    ...
		    get_finance get_crew_accounts get_crew_account add_crew_account 
		     change_crew_account del_crew_account get_oper_resources add_aud_resource change_aud_resource 
		     get_aud_resource add_ads_resource change_ads_resource get_ads_resource get_oper_offers 
		     get_oper_offer add_oper_offer change_oper_offer get_other_oper_offers get_oper_stats 
		     get_oper_profile change_oper_profile get_oper_finance get_oper_crew_accounts get_oper_crew_account 
		     add_oper_crew_account change_oper_crew_account del_oper_crew_account web_error);
...
# модуль описывающий интерфейс правобладателя и оператора
...
# Возвращает ссылку на массив списка ID ресурсов данного оператора
# на входе ID оператора 
sub get_oper_resources {
    my $oper_id = shift;
    # запрос к БД на выборку необходимых данных
    my $oper_aud_resources = _get_from_db_oper_resources($oper_id);
    my $json_text   = $json->encode($oper_aud_resources);
    return $json_text;
}

# Добавляет ресурс аудитории 
# возвращает ID-ресурса -  успех, 0 - ошибка
# на входе ID оператора и ссылка на хеш с данными ресурса
sub add_aud_resource {
    my ($oper_id, $aud_resource_hash) = @_;
    # вставляем информацию в БД
    my $result = _put_into_db_aud_resource($oper_id, $aud_resource_hash);
    my $json_text = $json->encode($result);
    # возвращаем строку в формате JSON
    return $json_text;
}

# Изменяет ресурс аудитории 
# возвращает 1 -  успех, 0 - ошибка
# на входе ID оператора и ссылка на хеш с данными ресурса
sub change_aud_resource {
    my ($oper_id, $aud_resource_hash) = @_;
    # вставляем информацию в БД
    my $result = _put_into_db_aud_resource($oper_id, $aud_resource_hash);
    my $json_text = $json->encode($result);
...
# Возвращает ссылку на хэш данных по ресурсу аудитории оператора 
# на входе ID оператора и ID ресурса 
sub get_aud_resource {
    my ($oper_id, $aud_resource_id) = @_;
    # запрос к БД на выборку необходимых данных
    my $aud_resource_hash = _get_from_db_aud_resource($oper_id, $aud_resource_id);
    my $json_text   = $json->encode($aud_resource_hash);
    # возвращает данные в формате JSON:
    return $json_text;
}
...
# возвращает ID-ресурса -  успех, 0 - ошибка
# на входе ID оператора и ссылка на хеш с данными ресурса
sub add_ads_resource {
    my ($oper_id, $ads_resource_hash) = @_;
    # вставляем информацию в БД
    my $result = _put_into_db_ads_resource($oper_id, $ads_resource_hash);
    my $json_text = $json->encode($result);
    # возвращаем строку в формате JSON
    return $json_text;
}
...
# Изменяет ресурс 
# возвращает 1 -  успех, 0 - ошибка
# на входе ID оператора и ссылка на хеш с данными ресурса рекламы
sub change_ads_resource {
    my ($oper_id, $ads_resource_hash) = @_;
    # вставляем информацию в БД
    my $result = _put_into_db_ads_resource($oper_id, $ads_resource_hash);
    my $json_text = $json->encode($result);
...
# Возвращает ссылку на хэш данных по ресурсу оператора 
# на входе ID оператора и ID ресурса рекламы
sub get_ads_resource {
    my ($oper_id, $ads_resource_id) = @_;
    # запрос к БД на выборку необходимых данных
    my $ads_resource_hash = _get_from_db_ads_resource($oper_id, $ads_resource_id);
    my $json_text   = $json->encode($ads_resource_hash);
    # возвращает данные в формате JSON:
    return $json_text;
}
...
# возвращает ссылку на массив со списком ID предложений сделанных оператором
# на входе передаем ID оператора
sub get_oper_offers {
    my $oper_id = shift;
    # запрос к БД на выборку необходимых данных
    my $oper_offers = _get_from_db_oper_offers($oper_id);
    my $json_text   = $json->encode($oper_offers);
    return $json_text;
}

# возвращает ссылку на хеш параметров предложения сделанного самим пользователем
# на входе передаем ID оператора  и ID предложения
sub get_oper_offer { 
    my ($oper_id, $offer_id) = @_;
    # запрос к БД на выборку необходимых данных
    my $oper_offer_hash = _get_from_db_oper_offer($oper_id, $offer_id);
    my $json_text   = $json->encode($oper_offer_hash);
    return $json_text;
}

# добавляет предложение оператора в базу, 
# возвращает ID предложения если удача, 0 если ошибка.
# на входе передаем ID оператора и хеш данных по предложению
sub add_oper_offer {
	my ($oper_id, $oper_offer_hash) = @_;
    # вставляем информацию в БД (INSERT)
    my $result = _put_into_db_oper_offer($oper_id, $oper_offer_hash);
    my $json_text   = $json->encode($result);
    return $json_text;
}

# изменяет предложение оператора в базе, 
# возвращает ID предложения если удача, 0 если ошибка
# на входе передаем ID оператора, ID предложения и хеш данных по предложению
sub change_oper_offer {
	my ($oper_id, $offer_id, $oper_offer_hash) = @_;
    # вставляем информацию в БД (UPDATE)
    my $result = _put_into_db_oper_offer($oper_id, $offer_id, $oper_offer_hash);
    my $json_text   = $json->encode($result);
    return $json_text;
}

# возвращает ссылку на массив со списком ID предложений сделанных друими операторами
# на входе передаем ID оператора
sub get_other_oper_offers {
	my $oper_id = shift;
    # запрос к БД на выборку необходимых данных
    my $oper_offers = _get_from_db_oper_other_offers($oper_id);
    my $json_text   = $json->encode($oper_offers);
    return $json_text;
}

# возвращает ссылку на хэш со cтатистической информацией по действиям пользователя
# на входе передаем ID оператора, период за который необходимо получить статистику и хеш других данных для выборки
sub get_oper_stats {
	my ($oper_id, $period, $other_terms_hash) = @_;
    # запрос к БД на выборку необходимых данных
    my $oper_stats = _get_from_db_oper_stats($oper_id, $period, $other_terms_hash);
    my $json_text   = $json->encode($oper_stats);
    return $json_text;
}


# возвращает ссылку на хэш с информацией по профайлу оператора
# на входе передаем ID оператора
sub get_oper_profile {
	my $oper_id = shift;
    # запрос к БД на выборку необходимых данных
    my $oper_profile = _get_from_db_oper_profile($oper_id);
    my $json_text   = $json->encode($oper_profile);
    return $json_text;
}


# меняет профайл оператора 
# возвращает 1 -  успех, 0 - ошибка
# на входе передаем ID оператора и хеш профиля оператора
sub change_oper_profile {
	my ($oper_id, $oper_profile_hash) = @_;
    # запрос к БД на выборку необходимых данных
    my $result = _put_into_db_oper_profile($oper_id, $oper_profile_hash);
    my $json_text   = $json->encode($result);
    return $json_text;
}

# возвращает ссылку на хэш с финансовой информацией по оператору
# функция обращается к локальной БД и Биллингу, и получает информацию о 
# балансе и совершенных ранее финансовых операциях за указанный период
# на входе передаем ID оператора и интересующий период
sub get_oper_finance {
	my ($oper_id, $period) = @_;
    # запрос к БД на выборку необходимых данных
    my $oper_finance = _get_from_db_oper_finance($oper_id, $period);
    my $json_text   = $json->encode($oper_finance);
    return $json_text;
}



# возвращает ссылку на массив со списом ID аккаунтов оператора 
# на входе передаем ID оператора
sub get_oper_crew_accounts {
	my $oper_id = shift;
    # запрос к БД на выборку необходимых данных
    my $oper_crew_accounts_id = _get_from_db_oper_accounts($oper_id);
    my $json_text   = $json->encode($oper_crew_accounts_id);
    return $json_text;
}

# возвращает ссылку на хэш перечнем настроек аккаунта оператора 
# на входе передаем ID оператора и ID аккаунта оператора
sub get_oper_crew_account {
	my ($oper_id, $oper_crew_account_id) = @_;
    # запрос к БД на выборку необходимых данных
    my $oper_crew_accounts = _get_from_db_oper_account($oper_id, $oper_crew_account_id);
    my $json_text   = $json->encode($oper_crew_accounts);
    return $json_text;
}

# добавяет аккаунт для оператора 
# возвращает ID аккаунта -  успех, 0 - ошибка
# на входе передаем ID оператора и хеш с данными по аккаунту оператора
sub add_oper_crew_account {
	my ($oper_id, $oper_crew_account_hash) = @_;
    # вставляем информацию в БД (INSERT)
    my $result = _put_into_db_oper_crew_account ($oper_id, $oper_crew_account_hash);
    my $json_text   = $json->encode($result);
    return $json_text;
}


# изменяет аккаунт для оператора
# возвращает ID аккаунта -  успех, 0 - ошибка
# на входе передаем ID оператора, ID аккаунта и хеш с данными по аккаунту оператора
sub change_oper_crew_account {
	my ($oper_id, $crew_account_id, $oper_crew_account_hash) = @_;
    # вставляем информацию в БД (UPDATE)
    my $result = _put_into_db_oper_crew_account ($oper_id, $crew_account_id, $oper_crew_account_hash);
    my $json_text   = $json->encode($result);
    return $json_text;
}

# удаляет аккаунт для оператора
# возвращает 1 -  успех, 0 - ошибка
# на входе передаем ID оператора и ID аккаунта
sub del_oper_crew_account {
	my ($oper_id, $crew_account_id) = @_;
    # вставляем информацию в БД (UPDATE)
    my $result = _del_from_db_oper_crew_account ($oper_id, $crew_account_id);
    my $json_text   = $json->encode($result);
    return $json_text;
}
...
#####################
#####################
sub _get_from_db_oper_resources {
	my $oper_id = shift;
	return [113,4565,77555,545345];
}

sub _put_into_db_aud_resource{
	my ($oper_id, $aud_recource_hash) = @_;	
	return [1112];
}

sub _get_from_db_aud_resource {
	my ($oper_id, $aud_resource_id) = @_;
    my %hash = (m_name     => 'Аудитория до 16',
                m_genre    => 'Боевик',
                m_year     => 'Colambia Pic/2009',
                m_director => 'МакДжи/Кристиан Бейл, Антон Ельчин, Сэм Уортингтон',
                m_trailers => '1.mpg,2.mpg,3.mpg',
...
sub _put_into_db_ads_resource {
	my ($oper_id, $ads_recource_hash) = @_;	
	return [1112];
}

sub _get_from_db_ads_resource {
	my ($oper_id, $ads_resource_id) = @_;
    my %hash = (m_name     => 'Рекламная площадка',
                m_genre    => 'Боевика',
                m_year     => 'Colambia Pic/2009',
                m_director => 'МакДжи/Кристиан Бейл, Антон Ельчин, Сэм Уортингтон',
                m_trailers => '1.mpg,2.mpg,3.mpg',
...
sub _get_from_db_oper_offers {
	my $oper_id = shift;
	return  [1030,1021,1627,8400];
}

sub _get_from_db_oper_offer {
	my ($oper_id, $offer_id) = @_;
	my %hash = (o_name   => 'Новогодние фильмы',
				o_type   => 'Дистрибуция',
				o_period => '05112009-31122009',
...
sub _put_into_db_oper_offer {
	my ($oper_id, $oper_offer_hash) = @_;
	return [14234];
}

sub _get_from_db_oper_other_offers {
	my $oper_id = shift;
	my %hash = (o_name   => 'Детские фильмы',
				o_type   => 'Дистрибуция',
				o_period => '01112009-10012010',
...
sub _put_into_db_oper_profile {
	my ($oper_id, $oper_profile_hash) = @_;
	return [1];
}


sub _get_from_db_oper_finance {
	my ($oper_id, $period) = @_;
	my %hash = (f_balance   => '7456.00',
				f_currency   => 'руб',
				f_period => '01112009-10012010',
...
sub _get_from_db_oper_accounts {
	my $oper_id = shift;
	return [113,4565,77555,545345];
}

sub _get_from_db_oper_account {
	my %hash = (a_name   => 'Иванов Петр Васильевич',
				a_rights   => '[1,1,0,1,0]',
				a_email => '01112009-10012010',
                a_creationdate => '01062009',
...
sub _del_from_db_oper_crew_account {
	my ($oper_id, $crew_account_id) = @_;
	return [1];
}

sub _put_into_db_oper_crew_account {
	my ($oper_id, $crew_account_id) = @_;
	return[12123];	
}
